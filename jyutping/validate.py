...
class Validator:
    ...
    def rules(*status):
        def generate(level):
            def fire(*matchers):
                return (level, matchers)
            return fire
        return tuple(generate(level) for level in status)
...
def validate_args(*args):
    ...
    2  # ValidationStatus.INVALID
    '''
    valid, alert, error = Validator.rules(ValidationStatus.VALID, ValidationStatus.UNCOMMON, ValidationStatus.INVALID)
